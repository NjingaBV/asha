...
5. Rédigez les deltas de spécification dans `changes/<id>/specs/<capability>/spec.md` (un dossier par capacité) en utilisant `## AJOUT|MODIFIE|SUPPRIME Requirements` avec au moins un `#### Scenario:` par exigence et en faisant référence aux capacités connexes le cas échéant. 6. Rédigez le fichier `tâches.md` sous forme de liste ordonnée de petites tâches vérifiables, permettant de visualiser la progression pour l'utilisateur, incluant la validation (tests, outils) et mettant en évidence les dépendances et les tâches parallélisables.
6. Validez avec `openspec validate <id> --strict` et corrigez chaque problème avant de soumettre la proposition.
